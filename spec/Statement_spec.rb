...
describe Statement do
  ...
  it "should add the transaction to the transactions array" do
    date = testdate
    expect { @statement.new_transaction(date, 100, 0, 100) }.to change{ @statement.transactions.length }.by(1)
  end
  ...
  it "should save every transaction as an array of arrays" do
    ...
    date2 = testdate2
    @statement.new_transaction(date, 100, 0, 100)
    expect { @statement.new_transaction(date2, 0, 50, 50) }.to change{ @statement.transactions.length }.by(1)
  end
end
